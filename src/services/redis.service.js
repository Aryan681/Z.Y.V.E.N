...
const redisService = {
  ...
  getAllPattern: async (pattern) => {
    try {
      ...
      logger.debug(
        `Redis pattern scan completed for pattern ${pattern}: found ${keys.length} keys`,
      );
      return keys;
    } catch (error) {
  ...
  getNestedPattern: async (key, field) => {
    try {
      ...
      logger.info(`Redis get nested key ${key} field ${field} successfully`);
      return value;
    } catch (error) {
      logger.error(`Redis get nested key ${key} error ${error}`);
      throw error;
    }
  },
